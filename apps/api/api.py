...
from apps.api.routers.organization import members_router, org_router, workspaces_router
...
_api_docs = NoncedSwagger() if settings.AGENT_API_DOCS_ENABLED else None

_OPENAPI_TAGS = [
    {
        "name": "system",
        "description": "Public liveness endpoints. No bearer token required.",
    },
    {
        "name": "me",
        "description": "Introspect the caller's API key scope, workspace, permissions, and storage quota.",
    },
    {
        "name": "config",
        "description": (
            "Runtime API configuration (dropdown enums, defaults, pagination). "
            "Edit `config/api_defaults.json` or set `API_CONFIG_PATH`; restart container to apply."
        ),
    },
    {
        "name": "accounts",
        "description": "Social accounts, OAuth connect URLs, disconnect. Allowlist-scoped.",
    },
    {
        "name": "posts",
        "description": "Create, read, update, schedule, and cancel posts. Workspace scope comes from the bearer token.",
    },
    {
        "name": "media",
        "description": "Upload, list, and retrieve media assets for use in post composition.",
    },
    {
        "name": "analytics",
        "description": "Read-only channel and post performance metrics (requires `view_analytics` permission).",
    },
    {
        "name": "calendar",
        "description": "Calendar events, queues, posting slots, reschedule, custom events.",
    },
    {
        "name": "inbox",
        "description": "Unified inbox: messages, replies, notes, bulk actions, saved replies, SLA.",
    },
    {
        "name": "approvals",
        "description": "Approval workflow: submit, approve, reject, request changes, bulk actions.",
    },
    {
        "name": "organization",
        "description": "Organization settings (requires org admin API key issuer).",
    },
    {
        "name": "workspaces",
        "description": "Workspace list/create and current workspace settings.",
    },
    {
        "name": "members",
        "description": "Team members, invitations, role assignments.",
    },
    {
        "name": "mcp",
        "description": (
            "Model Context Protocol (MCP) Streamable HTTP transport. JSON-RPC over POST. "
            "Accepts `bb_studio_` API keys or OAuth 2.1 access tokens. "
            "See docs/API.md for the full tool catalog."
        ),
    },
]
...
api = NinjaAPI(
    ...
    description=(
        ...
        "**Configuration.** Dropdown values and defaults come from "
        "`config/api_defaults.json` (override with `API_CONFIG_PATH`). "
        "Restart the app after changes — no image rebuild. See `GET /config/`."
    ),
    auth=ApiKeyAuth(),
    docs=_api_docs,
    docs_url="/docs",
...
api.add_router("/system", system_router, auth=None)
api.add_router("/config", config_router, auth=None)
...
api.add_router("/organization", org_router)
