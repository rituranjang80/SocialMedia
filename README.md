...
| [docs/API.md](docs/API.md) | **Agent API reference** — auth, endpoints, MCP tools, rate limits |
| [docs/FEATURES.md](docs/FEATURES.md) | Feature matrix: Web UI vs REST API vs MCP |
| [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md) | Docker, Kubernetes, one-click scripts, scaling |
| [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) | Stack, multitenant model, app map (LLM-friendly) |
| [AGENTS.md](AGENTS.md) | Quick orientation for coding agents / LLMs |

## Agent API & Swagger

Programmatic access for AI agents and integrations. **All REST endpoints are documented in Swagger.**

| URL | Purpose |
|-----|---------|
| `/api/v1/docs` | Interactive Swagger UI — try every endpoint |
| `/api/v1/openapi.json` | OpenAPI 3 schema (for codegen / LLMs) |
| `/api/v1/mcp` | MCP transport (JSON-RPC; tools mirror REST) |

**Local (after start):** http://localhost:8000/api/v1/docs

Authentication: `Authorization: Bearer bb_studio_<key>` — create keys at **Organization → API Keys** in the Web UI.

Swagger covers: **me**, **accounts**, **posts**, **media**, **analytics**, **mcp**, **system** (health).  
Calendar, inbox, approvals, and org admin are **Web UI only** — see [docs/FEATURES.md](docs/FEATURES.md).

Set `AGENT_API_DOCS_ENABLED=false` to hide Swagger in production.

## Quick Start (one-click)

**Fastest path** — starts Web UI, Agent API, Swagger, Postgres, Tailwind, and parallel workers:
...
Then open http://localhost:8000 and http://localhost:8000/api/v1/docs

Create admin: `docker compose exec app python manage.py createsuperuser`

See [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md) for Kubernetes (`scripts/start-k8s.*`) and production options.
...
See `docs/ARCHITECTURE.md` and `development_specs/architecture.md` for detailed per-platform instructions and cost breakdowns.
...
├── docs/                      # API, deployment, architecture docs
├── scripts/                   # One-click start (Docker, K8s)
├── AGENTS.md                  # LLM / coding agent quick guide
├── Dockerfile
...
When adding or changing **Agent API** endpoints, update Swagger-visible schemas in `apps/api/schemas.py` (with field descriptions) and document behavior in [docs/API.md](docs/API.md). Keep [docs/FEATURES.md](docs/FEATURES.md) in sync if feature coverage changes.
