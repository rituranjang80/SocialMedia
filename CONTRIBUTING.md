...
Maintain these when changing behavior:

| File | Update when |
|------|-------------|
| [docs/API.md](docs/API.md) | Agent API endpoints, auth, rate limits, MCP tools |
| [docs/FEATURES.md](docs/FEATURES.md) | Feature coverage (Web UI vs API) changes |
| [docs/DEPLOYMENT.md](docs/DEPLOYMENT.md) | Docker, K8s, env vars, scripts |
| [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) | Stack, apps, multitenant model |
| [AGENTS.md](AGENTS.md) | Conventions for LLM/coding agents |
| [README.md](README.md) | User-facing quick start, Swagger links |

**Swagger:** All Agent API routes live under `apps/api/routers/`. Every endpoint needs a `summary=` and Pydantic schemas with `Field(description=...)`. Verify at `/api/v1/docs` after changes.
