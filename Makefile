.PHONY: help setup dev server worker tailwind test lint format typecheck migrate migrations docker-up docker-down docker-build run run-k8s
...
run: ## One-click: start UI + API + Swagger + parallel workers (Docker)
	@bash scripts/start.sh

run-k8s: ## One-click: deploy to Kubernetes (UI + API + Swagger + workers)
